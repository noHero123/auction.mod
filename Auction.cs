...
namespace UserMenuInChat.mod
{
    ...
    public class Auction : BaseMod, ICommListener, iEffect, iCardRule, ICardListCallback
    {
        ...
        private void getaucitemsformmsg(string msgg, string from, string room)
        {
            ...
            for (int i = 0; i < words.Length; i++)
            {
                ...
                // save in wts or wtb?
                if (word.ToLower().Contains("wts") || word.ToLower().Contains("sell")) { wts = true; }
                if (word.ToLower().Contains("wtb") || word.ToLower().Contains("buy")) { wts = false; }
                       ...
                       /* if (current.card.level > 0)
                        ...
                        if (this.wtsmenue)
                        {
                            if (GUI.Button(new Rect(position7.xMax+2, (float)num * this.fieldHeight, this.costIconWidth, this.fieldHeight), "SP"))
                            {
                                ...
                                this.SPtarget = true;
                                this.PriceCheck(current.card.getName());
                            }
                        }
                        else
                        {
    ...
    //icardlistcallback
        ...
        {// set rects for menus
            ...
            float chatheight = this.chatLogStyle.CalcHeight(new GUIContent("Scroll"), 1000);
            ...
            this.tbmessage = new Rect(this.tradingbox.x, this.tradingbox.y, this.tradingbox.width, this.tradingbox.height - (float)Screen.height * 0.05f);
        }
